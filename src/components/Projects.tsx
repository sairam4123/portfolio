      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {projects.map((p) => (
            <div
              key={p.name}
              className="group bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 flex flex-col hover:border-blue-500/40 hover:bg-white/[0.07] hover:shadow-[0_0_40px_rgba(59,130,246,0.08)] transition-all duration-300"
            >
              {/* Card header */}
              <div className="flex items-start justify-between mb-4">
                <FolderOpen
                  size={28}
                  className="text-blue-400/70 group-hover:text-blue-400 transition-colors duration-200"
                />
                <a
                  href={p.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[#8892b0] hover:text-blue-400 transition-colors duration-200"
              ...
              <h3 className="text-[#ccd6f6] font-semibold text-lg mb-2 group-hover:text-blue-300 transition-colors duration-200">
                {p.name}
              </h3>
              ...
              {p.tech.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {p.tech.map((t) => (
                    <span
                      key={t}
                      className="px-2 py-0.5 text-xs bg-blue-600/10 border border-blue-500/20 text-blue-300 rounded-full font-mono"
                    >
                      {t}
                    </span>
                  ))}
