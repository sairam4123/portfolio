...
const achievements: { icon: LucideIcon; color: string; bg: string; border: string; content: React.ReactNode }[] = [
  {
    icon: Trophy,
    color: "text-yellow-400",
    bg: "bg-yellow-500/10",
    border: "border-yellow-500/20",
    content: <>Won {hi("10+ prizes")} across {hi("12+ symposiums")}.</>,
  },
  {
    icon: BookOpen,
    color: "text-sky-300",
    ...
    content: <>Published a research paper on {hi("TRAX")} in the {hi("IJMRSET")}.</>,
  },
  {
    icon: Award,
    color: "text-cyan-300",
    ...
    content: <>Received {hi("2× Best Technical Student Award")} at Mookambigai College of Engineering.</>,
  },
  {
    icon: Gamepad2,
    ...
    content: <>Organized {hi("5 GIC Game Jams")} for the Godot India Community.</>,
  },
];
          ...
          {achievements.map((a, i) => (
            <StaggerItem key={i} index={i}>
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 flex gap-5 items-start hover:border-white/20 hover:bg-white/[0.07] transition-all duration-300">
                ...
                <p className="text-[#8892b0] text-sm leading-relaxed pt-3">{a.content}</p>
              </div>
            </StaggerItem>
          ))}
        </Stagger>
